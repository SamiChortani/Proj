      ...
      <div className="container mx-auto px-2">
        <div className="grid grid-cols-2 md:grid-cols-7 xl:grid-cols-12 gap-5 sm:gap-9 lg:gap-7 py-10 justify-between ">
              ...
              <ul className="text-sm flex flex-col space-y-3">
                {link.links.map((text, index) => (
                  <li key={index} className="flex items-baseline ">
                    <Link
                      to={text.link}
                      className="text-border inline-block w-full hover:text-star "
                    >
                      {text.name}
                    </Link>
                  </li>
          ...
          <div className="pb-3.5 sm:pb-0 col-span-1 md:col-span-2 lg:col-span-3">
            <Link to="/">
              <img src={BL} alt="logo" className="w-2/4 " />
              {/*object-contain h-12*/}
            </Link>
            <p className="leading-7 text-sm text-border mt-3">
              ...
              <br/>
              <span>PHONE: +19999999</span>
              <br/>
              <span>Email: chortani666@gmail.com </span>
            </p>
          </div>
        </div>
      </div>
